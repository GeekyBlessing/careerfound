...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-base-950">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-white/[0.06] bg-base-900/60 lg:flex">
        ...
        <nav className="flex-1 space-y-1 px-3 py-4">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? "bg-accent/15 text-accent-light" : "text-ink-300 hover:bg-white/[0.05] hover:text-ink-100"
                )}
              >
          ...
          {user.role === "mentor" && (
            <Link
              ...
              className={cn(
                "flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors",
                pathname?.startsWith("/mentor-dashboard") ? "bg-accent/15 text-accent-light" : "text-ink-300 hover:bg-white/[0.05] hover:text-ink-100"
              )}
            >
          ...
          {user.role === "admin" && (
            <Link
              ...
              className={cn(
                "flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors",
                pathname?.startsWith("/admin") ? "bg-accent/15 text-accent-light" : "text-ink-300 hover:bg-white/[0.05] hover:text-ink-100"
              )}
            >
              <GraduationCap className="h-4 w-4" />
              Admin
        ...
        <div className="border-t border-white/[0.06] p-4">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-accent/20 text-sm font-semibold text-accent-light">
            ...
            <button onClick={logout} aria-label="Log out" className="text-ink-500 hover:text-ink-100 focus-ring rounded-lg p-1.5">
...
function MobileTopBar() {
  ...
  return (
    <div className="flex h-14 items-center justify-between border-b border-white/[0.06] px-4 lg:hidden">
      ...
      <button onClick={logout} className="text-ink-400">
        <LogOut className="h-4 w-4" />
      </button>
    </div>
  );
